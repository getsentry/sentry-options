...
- Features with a `created_at` after 2026-10-15 (UTC) bucket by feature name as well: their id is the same hash over `name:key:value:...`, so two such features at the same rollout reach different orgs, and the same orgs no longer land in the low buckets of every new flag. Features created at or before that date, or with a `created_at` that does not parse, keep the identity-only id so their in-flight rollouts do not move. Sentry's Python `flagpole` package pins the same epoch and hash.
