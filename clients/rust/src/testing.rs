...
mod tests {
    ...
    fn checkout_layer(members: Vec<(&str, u32, u32)>) -> Value {
    ...
    #[test]
    fn test_override_with_overlapping_layer_is_rejected() {
        crate::init().unwrap();
        match override_options(&[(
            "sentry-options-testing",
            "experiment-layer.checkout",
            checkout_layer(vec![("checkout-color", 0, 50), ("checkout-copy", 30, 30)]),
        )]) {
        ...
        assert!(
            override_options(&[(
                "sentry-options-testing",
                "experiment-layer.checkout",
                checkout_layer(vec![("checkout-color", 0, 40), ("checkout-copy", 40, 30)]),
            )])
            .is_ok()
        );
    }
    ...
    #[test]
    fn test_override_overlapping_layer_is_rejected_and_restores() {
        crate::init().unwrap();
        let ns = "sentry-options-testing";
        let result = override_options(&[(
            ns,
            "experiment-layer.checkout",
            checkout_layer(vec![("checkout-color", 0, 50), ("checkout-copy", 40, 30)]),
        )]);
        match result {
            Err(e) => assert!(e.to_string().contains("overlap"), "{e}"),
            Ok(_) => panic!("overlapping layer should be rejected"),
        }
    ...
    #[test]
    fn test_override_can_reallocate_siblings() {
        crate::init().unwrap();
        let ns = "sentry-options-testing";
        let _guard = override_options(&[(
            ns,
            "experiment-layer.checkout",
            checkout_layer(vec![("checkout-color", 0, 50), ("checkout-copy", 50, 30)]),
        )])
