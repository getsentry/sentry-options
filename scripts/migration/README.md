...
If `active_options.json` exists in the same directory as the script (generated by
`enumerate_active_options.py`), it is merged automatically. Runtime-only and DB-only
options are included in the CSV with `safe_to_delete` values of `RUNTIME_ONLY` and
`DB_ONLY` respectively. Pass `--active-options PATH` to use a snapshot from a different
location.
